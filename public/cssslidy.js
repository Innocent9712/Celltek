var cssSlidy = function (newOptions) {
  var options = (function () {
      var mergedOptions = {},
        defaultOptions = {
          ...
          captionPosition: "top", // options: bottom, top
  ...
  CS.init = (function () {
    ...
    // browser supports keyframe w/ prefixes
    ...
    var imgCount = images.length, // count the number of images in the slide, including the new cloned element
      totalTime =
        (options.timeOnSlide + options.timeBetweenSlides) * (imgCount - 1),
      // calculate the total length of the animation by multiplying the number of ACTUAL images by the amount of time for both static display of each image and motion between them;
      slideRatio = (options.timeOnSlide / totalTime) * 100,
      // determines the percentage of time an individual image is held static during animation
      moveRatio = (options.timeBetweenSlides / totalTime) * 100,
      // determines the percentage of time for an individual movement
    ...
    // set the width of the inner image
    if (
      options.captionSource == "alt" ||
      options.captionSource == "data-caption"
    ) {
    ...
    if (options.slidyDirection == "right") {
      ...
      for (var i = imgCount - 1; i > 0; i--) {
        ...
        position += moveRatio;
        // make the position for the NEXT slide
        css.innerHTML += position + "% { left: -" + (i - 1) * 100 + "%; }\n";
      }
    } else {
      css.innerHTML += position + "% { left: -" + 0 * 100 + "%; }\n";
